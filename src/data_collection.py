...
def process_data(df: pd.DataFrame):
    ...
    step_size = 2
    for index in tqdm(range(480, len(df), step_size), desc = "Generating completions"):
        ...
        save_to_csv(df, "merged_unsafe_prompts_ongoing_contexts")
    return df
...
save_to_csv(dataset_postprocessed, "rerun_unsafe_prompts_contexts_completion_postprocessed")